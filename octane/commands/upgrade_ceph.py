...
def upgrade_ceph(orig_id, seed_id):
    ...
    filename = os.path.join(magic_consts.FUEL_CACHE,
                            "env-{0}-ceph.conf.tar.gz".format(orig_id))
    conf_filename, db_path = extract_mon_conf_files(controller, filename)
    ceph_set_new_mons(controllers, filename, conf_filename, db_path)
...
def short_hostname(hostname):
    return hostname.strip().partition('.')[0]


def extract_mon_conf_files(node, filename):
    conf_filename = get_ceph_conf_filename(node)
    conf_dir = os.path.dirname(conf_filename)
    hostname = short_hostname(node_util.get_hostname_remotely(node))
    db_path = "/var/lib/ceph/mon/ceph-{0}".format(hostname)
    node_util.tar_files(filename, node, conf_dir, db_path)
...
def ceph_set_new_mons(nodes, filename, conf_filename, db_path):
    ...
    for node_hostname, node_ip in itertools.izip(hostnames, mgmt_ips):
        monmaptool_cmd.extend(('--add', node_hostname, node_ip))
    monmaptool_cmd.append('/tmp/monmap')
