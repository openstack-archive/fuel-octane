...
def upgrade_node(env_id, node_ids, isolated=False, network_template=None,
    ...
    # Sanity check
    one_orig_id = None
    node_roles = collections.defaultdict(list)
    for node in nodes:
    ...
    if len(node_roles['ceph-osd']) > 1 and live_migration:
        raise Exception(
