...
def upgrade_node(env_id, node_ids, isolated=False, network_template=None,
                 provision=True, roles=None, no_live_migration=False):
    ...
    if len(node_roles['compute']) > 1 and not no_live_migration:
        raise Exception(
            "You can't upgrade nore then 1 compute in same time "
            "compute nodes are: {compute} \n"
            "Mark --no-live-migration to run this "
            "command on multiple nodes".format(node_roles)
        )
    if len(node_roles['ceph-osd']) > 1 and not no_live_migration:
        raise Exception(
    ...
    call_handlers = upgrade_handlers.get_nodes_handlers(
        nodes, env, isolated, no_live_migration)
...
class UpgradeNodeCommand(cmd.Command):
    ...
    def take_action(self, parsed_args):
        upgrade_node(parsed_args.env_id, parsed_args.node_ids,
                     ...
                     no_live_migration=parsed_args.no_live_migration)
