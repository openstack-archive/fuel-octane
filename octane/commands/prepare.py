...
def prepare(ibp=False):
    ...
    # If IBP was used to install the original cluster, don't patch nailgun and
    # initramfs.
    if not ibp:
        apply_patches()
...
def revert_prepare(ibp=False):
    # If IBP was used to install the original cluster, there's no need to
    # revert anything.
    if not ibp:
