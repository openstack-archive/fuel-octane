...
def upgrade_control_plane_with_graph(orig_id, seed_id):
    ...
    try:
        ...
        deploy.execute_graph_and_wait('switch-controlplane-1', seed_id)

        # Stop clusetered services on the original controllers
        deploy.execute_graph_and_wait('switch-controlplane-1', orig_id)
        ...
        deploy.execute_graph_and_wait('switch-controlplane-2', seed_id)

        # Decommission the orig controllers by stopping OpenStack services
        deploy.execute_graph_and_wait('switch-controlplane-2', orig_id)

    except: Exception
        # TODO(pchechetin): Disconnect the orig controller
        #                   Connect the seed controller
        LOG.info('Trying to rollback switch-controlplane phase')

        deploy.execute_graph_and_wait('switch-controlplane-rollback', orig_id)
        deploy.execute_graph_and_wait('switch-controlplane-rollback', seed_id)
...
class UpgradeControlPlaneCommand(cmd.Command):
    ...
    def take_action(self, parsed_args):
        if parsed_args.with_graph:
            upgrade_control_plane_with_graph(parsed_args.orig_id, parsed_args.seed_id)
        else:
