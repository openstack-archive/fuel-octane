...
def upgrade_db(orig_id, seed_id, db_role_name):
    ...
    for f_name in file_names:
        ...
        remote_name = os.path.join(magic_consts.NOVA_PATCH_PREFIX_DIR, f_name)
    ...
    try:
        ...
        patch.patch_apply(tempdir_path, magic_consts.NOVA_PATCHES)
        for node in nodes:
        ...
        try:
            ...
            patch.patch_apply(
                tempdir_path, magic_consts.NOVA_PATCHES, revert=True)
            for node in nodes:
                ssh.put_files_to_remote_node(
                    node, [(b, a) for a, b in file_name_pairs])
    finally:
