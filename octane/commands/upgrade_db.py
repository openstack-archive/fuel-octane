...
def upgrade_db_with_graph(orig_id, seed_id):
    ...
    # Execute on original environment
    ...
    # Execute on seed environment
    if not dutil.graph_execute_and_wait("upgrade-db-seed", seed_id):
        if 'upgrade-db-seed-rollback' in dutil.graph_list(seed_id):
            dutil.graph_execute_and_wait("upgrade-db-seed-rollback", seed_id)
        return
