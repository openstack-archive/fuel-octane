...
def restore_admin_node(path_to_backup):
    context = backup_restore.Context(password='')
    with contextlib.closing(tarfile.open(path_to_backup)) as archive:
        archivators = [cls(archive) for cls in backup_restore.REPO_ARCHIVATORS]
...
class RestoreRepoCommand(command.Command):

    def get_parser(self, *args, **kwargs):
        parser = super(RestoreRepoCommand, self).get_parser(*args, **kwargs)
        parser.add_argument(
            "--from",
            type=str,
            action="store",
