...
def test_create_links_on_remote_logs(mocker, mock_open, nodes, is_dir):
    ...
    for idx, node_link_exits in enumerate(nodes):
        ...
        sql_return_value.append((node_domain_name, ip_addr))
        is_link_exists.append(link_exists)
        if not link_exists:
            moved_nodes.append((node_domain_name, ip_addr))
    mocker.patch("os.path.islink", side_effect=is_link_exists)
    ...
    assert [mock.call(d, i) for d, i in path_pairs] == \
        symlink_mock.call_args_list
