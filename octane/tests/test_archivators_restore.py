...
def test_post_restore_action_astute(mocker):
    ...
    start = mocker.patch(
        "octane.util.docker.start_container", side_effect=foo(False))
    stop = mocker.patch(
        "octane.util.docker.stop_container", side_effect=foo(True))
...
def test_post_restore_nailgun(mocker):
    ...
    post_data.assert_has_calls([
        mock.call(post_url, '{"p": 2, "k": 1}', headers=headers),
        mock.call(post_url, '{"p": 2, "k": 3}', headers=headers),
    ])
