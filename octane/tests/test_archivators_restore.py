...
def test_postgres_restore(mocker, cls, db, sync_db_cmd):
    ...
    mocker.patch("octane.util.systemd.stop_container",
                 side_effect=foo("stop_container"))
    mocker.patch("octane.util.systemd.start_container",
                 side_effect=foo("start_container"))
    mock_timeout = mocker.patch(
        "octane.util.systemd.set_systemctl_start_timeout")
    mock_timeout.return_value.__enter__.side_effect = foo("set_timeout")
    mock_timeout.return_value.__exit__.side_effect = foo("unset_timeout")
    cls(archive).restore()
    member.assert_extract()
    assert ["stop_container", "run_in_container",
            "in_container", "set_timeout", "start_container",
            "unset_timeout", "run_in_container"] == actions
...
def test_post_restore_action_astute(mocker):
    ...
    start_service = mocker.patch("octane.util.systemd.start_container",
                                 side_effect=stopped.remove)
    stop_service = mocker.patch("octane.util.systemd.stop_container",
