...
def test_create_repo_preferences(mocker, params, repo, filename, content):
    mock_fetch_release = mocker.patch(
        'diesel.util.apt.fetch_release_parameters')
...
class MockTarFile(object):
    ...
    def addfile(self, tarinfo, fileobj=None):
        content = fileobj.read(tarinfo.size)
        self.files.append({'name': tarinfo.name,
                           'size': tarinfo.size,
                           'mtime': tarinfo.mtime,
                           'content': content})


def test_create_config_archive(mocker):
    mock_tarfile = MockTarFile()
    mock_taropen = mocker.patch('tarfile.TarFile.open')
    mock_taropen.return_value = mock_tarfile
    mocker.patch('tarfile.TarInfo', new=MockTarInfo)
    mock_repo_source = mocker.patch('diesel.util.apt.create_repo_source')
    mock_repo_source.side_effect = [('filename_one', 'content_one'),
                                    ('filename_two', 'content_two')]
    mock_repo_prefs = mocker.patch('diesel.util.apt.create_repo_preferences')
    mock_repo_prefs.side_effect = [('filename_three', 'content_three'),
                                   ('filename_four', 'content_four')]
    mock_time = mocker.patch('time.time')
    mock_time.return_value = mtime = 1020406080
    apt.create_config_archive('/home/my/tarball', REPOS)
    assert mock_tarfile.files == [
        {'name': 'filename_one', 'size': 11, 'mtime': mtime,
         'content': 'content_one'},
        {'name': 'filename_two', 'size': 11, 'mtime': mtime,
         'content': 'content_two'},
        {'name': 'filename_three', 'size': 13, 'mtime': mtime,
         'content': 'content_three'},
        {'name': 'filename_four', 'size': 12, 'mtime': mtime,
