...
def test_check_plugin():
    assert plugin.plugin_is_installed(CLUSTER_SETTINGS_SAMPLE,
                                      'emc_vnx') is True
    assert plugin.plugin_is_enabled(CLUSTER_SETTINGS_SAMPLE,
                                    'emc_vnx') is True

    assert plugin.plugin_is_installed(CLUSTER_SETTINGS_SAMPLE,
                                      'zabbix_monitoring_emc') is True
    assert plugin.plugin_is_enabled(CLUSTER_SETTINGS_SAMPLE,
                                    'zabbix_monitoring_emc') is False

    assert plugin.plugin_is_installed(CLUSTER_SETTINGS_SAMPLE,
                                      'non_existent_plugin') is False
    assert plugin.plugin_is_enabled(CLUSTER_SETTINGS_SAMPLE,
                                    'non_existent_plugin') is False
...
CLUSTER_SETTINGS_SAMPLE = {
    'editable': {
        'emc_vnx': {
            'emc_username': {
                'regex': {
                    'source': 'S', 'error': 'Username field cannot be empty'
                },
                'description': 'EMC VNX username.', 'weight': 75,
                'type': 'text',
                'value': 'username',
        ...
        'zabbix_monitoring_emc': {
            ...
            'metadata': {
                'restrictions': [{
                    'action': 'disable',
                    'message': 'This plugin requires SNMP trap daemon',
                    'condition': 'settings:zabbix_snmptrapd.enabled == false'
                }],
                'toggleable': True,
                'weight': 70,
                'plugin_id': 3,
