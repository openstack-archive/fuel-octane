...
@pytest.mark.parametrize("repos,priority,result", [
    ...
    (
        ...
        1000,
        'name=ubuntu-updates packages=libcephfs1 librados2 librbd1 '
        'python-ceph python-cephfs python-rados python-rbd ceph ceph-common '
        'ceph-fs-common ceph-mds priority=1000 section=main universe '
        'multiverse suite=trusty-updates type=deb uri=http://ubuntu/\n\n'
        'name=ubuntu packages=libcephfs1 librados2 librbd1 python-ceph '
        'python-cephfs python-rados python-rbd ceph ceph-common '
        'ceph-fs-common ceph-mds priority=9999 section=main universe '
        'multiverse suite=trusty type=deb uri=http://ubuntu/'
    ),
])
def test_generate_preference_pin(mocker, repos, priority, result):
    ...
    def foo(repo, packages):
        ...
        results = ' '.join('{0}={1}'.format(*i) for i in sorted(repo.items()))
        return None, results
