...
def test_get_sftp(mocker, username):
    ...
    mtransport = mclient().get_transport()
    mchannel = mtransport.open_channel()

    mtransport.auth_handler.username = username

    ssh._get_sftp(mocker.Mock(data={'id': 1}))

    if username == 'root':
        msftp_client.from_transport.assert_called_with(mtransport)
    else:
