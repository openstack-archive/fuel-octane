...
def test_repos_backup(
    ...
    docker_mock = mocker.patch("octane.util.docker.in_container")
    test_archive = mocker.Mock()
    path = "/var/www/nailgun/"
    docker_in_container = docker_mock.return_value.__enter__.return_value
    docker_in_container.communicate.return_value = sql_output, None
    cls(test_archive).backup()
    yaml_mocker.assert_called_once_with(mock_open.return_value)
    docker_mock.assert_called_once_with(
        "postgres", ["sudo", "-u", "postgres", "psql", "nailgun"],
        stdin=subprocess.PIPE, stdout=subprocess.PIPE
    )
    docker_in_container.stdin.write.assert_called_once_with(sql)
