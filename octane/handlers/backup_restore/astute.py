...
from octane.handlers.backup_restore import exceptions
...
class AstuteArchivator(base.PathArchivator):
    ...
    def restore(self):
        ...
        if not os.path.exists(self.path):
            raise exceptions.RestoreException("no astute etc file")
        ...
        if not_found_keys:
            raise exceptions.RestoreException(
                "Not found values in backup for keys: {0}".format(
                    ",".join(not_found_keys)))
        old_dump_name = "{0}.old".format(self.path)
        new_path_name = "{0}.new".format(self.path)
        shutil.copy(self.path, old_dump_name)
