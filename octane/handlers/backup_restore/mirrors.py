...
class NaigunWWWBackup(base.PathArchivator):
    ...
    def backup(self):
        ...
        with docker.in_container(
                "postgres", ["sudo", "-u", "postgres", "psql", self.db],
                stdin=subprocess.PIPE, stdout=subprocess.PIPE) as psql_runner:
            psql_runner.stdin.write(self.sql)
            results, _ = psql_runner.communicate()
            results = results.strip().strip("-").split("\n")[2:-1]
        already_backuped = set()
        for line in results:
            data = json.loads(line)
            for value in self._get_values_list(data):
                if ipaddr in value['uri']:
                    path = urlparse.urlsplit(value['uri']).path[1:]
                    dir_name = path.lstrip("/").split(os.path.sep, 1)[0]
