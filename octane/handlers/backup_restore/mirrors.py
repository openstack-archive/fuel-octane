...
class NaigunWWWBackup(base.PathArchivator, postgres.ContainerSQLRunningMixin):
    ...
    def backup(self):
        ...
        rows = sql.run_sql_on_container(self.sql, self.container, self.db)
        for line in rows:
...
class FullMirrorsBackup(NaigunWWWBackup):
    ...
    def _get_mirrors(self):
        results = sql.run_sql_on_container(self.sql, self.container, self.db)
