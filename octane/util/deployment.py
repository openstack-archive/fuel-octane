...
def upload_graph_file_to_env(graph_file_path, env_id):
    ...
    LOG.info("Graph '%s' uploaded for environment '%s'.", graph_name, env_id)


def upload_graphs_to_env(directory, env_id):
    """Upload all YAML-files as graph to an environment."""
...
def execute_graph_and_wait(graph_name, env_id,
    ...
    for i in xrange(attempts):
        ...
        LOG.info("Graph '%s' for environment %s has status '%s'",
                 graph_name, env_id, status)
        time.sleep(attempt_delay)
    raise Exception("Timeout waiting of {0} seconds for task {1} execution "
                    "for graph {2}."
