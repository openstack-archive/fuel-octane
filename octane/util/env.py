...
def change_env_settings(env_id, master_ip=''):
    ...
    for key, value in generated_data['provision']['image_data'].viewvalues():
        value['uri'] = release_image_data_dict[key].format({
            "settings.MASTER_IP": master_ip,
            "cluster.id": env_id,
        })
    sql.run_sql_on_container(
