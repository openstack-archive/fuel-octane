...
def fuel2_env_call(args, fail_on_error=True, output=False):
    ...
    try:
        ...
        LOG.warn("Fuel2 client returned non-zero exit code "
                 "for following arguments: %s", ' '.join(cmd))

        if fail_on_error:
            raise Exception("Fuel2 client returned non-zero "
                            "exit code for following arguments: %s" %
                            ' '.join(cmd))
