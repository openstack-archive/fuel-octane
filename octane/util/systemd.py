...
TIMEOUT_FILE = "/etc/systemd/{0}.conf.d/99-octane-timeout.conf"
TIMEOUT_CONF = ("[Service]\n"
                "TimeoutStartSec={0}\n")


@contextmanager
def set_systemctl_start_timeout(service, timeout):
    confdir = os.path.dirname(TIMEOUT_FILE.format(service))
    if not os.path.isdir(confdir):
        os.makedirs(confdir)
    with open(os.path.join(TIMEOUT_FILE.format(service)), 'w') as f:
