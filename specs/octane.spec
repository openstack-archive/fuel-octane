...
%install
cd %{_builddir}/%{name}-%{version} && python setup.py install --single-version-externally-managed -O1 --root=$RPM_BUILD_ROOT --record=%{_builddir}/%{name}-%{version}/INSTALLED_FILES && find octane/patches/ -type f >> %{_builddir}/%{name}-%{version}/INSTALLED_FILES
