...
%install
cd %{_builddir}/%{name}-%{version} && python setup.py install --single-version-externally-managed -O1 --root=$RPM_BUILD_ROOT --record=%{_builddir}/%{name}-%{version}/INSTALLED_FILES && cp -vr octane/patches ${RPM_BUILD_ROOT} && echo /octane/patches/ >> %{_builddir}/%{name}-%{version}/INSTALLED_FILES && ls -l ${RPM_BUILD_ROOT} 
